...
Refer to [Local development and dev workspace](../usecase/resources/README.md#local-development-and-dev-workspace) 
...
* Run `databricks configure --profile usecase-dev --token --host <your-dev-workspace-url>`, passing the URL of your dev workspace.
...
* From within the root directory of the current project, use the [dbx sync](https://dbx.readthedocs.io/en/latest/guides/python/devloop/mixed/#using-dbx-sync-repo-for-local-to-repo-synchronization) tool to copy code files from your local machine into the Repo by running
  `dbx sync repo --profile usecase-dev --source . --dest-repo your-repo-name`, where `your-repo-name` should be the last segment of the full repo name (`/Repos/username/your-repo-name`)

#### Running code on Databricks
You can iterate on the sample ML code by running the provided `usecase/training/notebooks/Train.py` notebook on Databricks using
