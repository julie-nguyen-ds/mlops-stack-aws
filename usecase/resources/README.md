...
The databricks CLI bundles top level is defined by file `usecase/databricks.yml`.
...
ML Resource Configurations in this directory:
 - model workflow (`usecase/resources/model-workflow-resource.yml`)
 - batch inference workflow (`usecase/resources/batch-inference-workflow-resource.yml`)
 - monitoring workflow (`usecase/resources/monitoring-workflow-resource.yml`)
 - feature engineering workflow (`usecase/resources/feature-engineering-workflow-resource.yml`)
 - model definition and experiment definition (`usecase/resources/ml-artifacts-resource.yml`)
...
If you want to use different catalog names, please update the `targets` declared in the `usecase/databricks.yml` and `usecase/resources/ml-artifacts-resource.yml` files.
...
|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------|-------------------------------------|------------------------------------------------|
| dev         | The `dev` deployment target is used by ML engineers to deploy ML resources to development workspace with `dev` configs. The config is for ML project development purposes.                                                           | dev workspace        | dev-usecase-model     | /dev-usecase-experiment     |
| staging     | The `staging` deployment target is part of the CD pipeline. Latest main content will be deployed to staging workspace with `staging` config.                                                             | staging workspace    | staging-usecase-model | /staging-usecase-experiment |
| prod        | The `prod` deployment target is part of the CD pipeline. Latest release content will be deployed to prod workspace with `prod` config.                                                                      | prod workspace       | prod-usecase-model    | /prod-usecase-experiment    |
| test        | The `test` deployment target is part of the CI pipeline. For changes targeting the main branch, upon making a PR, an integration test will be triggered and ML resources deployed to the staging workspace defined under `test` deployment target. | staging workspace    | test-usecase-model    | /test-usecase-experiment    |
...
2. Complete the `TODO` in `usecase/databricks.yml` to add the dev workspace URI under `targets.dev.workspace.host`.
...
Following this, update the batch_inference_job base parameters in `usecase/resources/batch-inference-workflow-resource.yml` to pass
...
To get started, open `usecase/resources/batch-inference-workflow-resource.yml`.  The file contains the ML resource definition of a batch inference job, like:
...
resources:
  jobs:
    batch_inference_job:
      name: ${bundle.target}-usecase-batch-inference-job
      tasks:
...
The example above defines a Databricks job with name `${bundle.target}-usecase-batch-inference-job`
that runs the notebook under `usecase/deployment/batch_inference/notebooks/BatchInference.py` to regularly apply your ML model for batch inference. 
...
We specify a `batch_inference_job` under `resources/jobs` to define a databricks workflow with internal key `batch_inference_job` and job name `{bundle.target}-usecase-batch-inference-job`.
...
`test-usecase-batch-inference-job`. During the deployment of the `staging` environment config, the job name will be
`staging-usecase-batch-inference-job`.
